...
public class FileUtil {
    ...
    public static String uploadFile(String rootPath, MultipartFile file) {
        ...
        // fullPath = http://localhost:8383/local/2024/06/05/(파일명).jpg

        // url-path= /local/2024/06/05/(파일명).jpg
        String urlPath = "/local" + fullPath.substring(rootPath.length());// rootPath의 길이부터 끝까지

        // 업로드가 완료되면 데이터베이스 파일에 경로 위치를 저장
        // ex) /local/2024/06/05/asldgjksddfgs_dog.jpg
        return urlPath;
    }
    ...
    private static String makeDateFormatDirectory(String rootPath) {
        ...
        List<String> dataLst = List.of(year + "" , len2(month), len2(day));

        String newDirectoryPath = rootPath;
        // newDirectoryPath - /spring-prj/upload/2024/06/05
        for (String s : dataLst) {
            newDirectoryPath += "/" + s;
