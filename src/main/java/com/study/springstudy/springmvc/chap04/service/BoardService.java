...
public class BoardService {
    ...
    public boolean insert(BoardWriteRequestDto dto, HttpSession session) {
        ...
        b.setAccount(getLoggedUserAccount(session));
    ...
    public BoardDetailResponseDto detail(int bno,
        ...
        // 로그인한 사람 계정
        String currentUserAccount = getLoggedUserAccount(session);
...
//        return new BoardDetailResponseDto(b);
        ...
        int boardNo = b.getBoardNo(); // 게시글 번호
        ...
        if (viewLog == null) {
            // 2. 이 게시물이 이 회원에 의해 처음 조회된 상황.
            viewLogMapper.insertViewLog(viewLogEntity);
        shouldIncrease = true;
        } else {
            // 3. 조회기록이 있는 경우 - 1시간 이내 인지, 아닌지 체크
            // 혹시 1시간이 지난 게시물인지 확인
